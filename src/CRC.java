...
public class CRC {
    public static void main(String[] args) throws IOException {
        ...
        int[] app_message; // dividend ka array
        int[] rem;
        int[] trans_message;
        ...
        message_bits = Integer.parseInt(br.readLine()); // jo message encode krke bhejna hai usmein total bits kitne hai

        message = new int[message_bits]; // message ka array
        System.out.println("Enter message:");
        for(i=0; i<message_bits; i++)
            message[i] = Integer.parseInt(br.readLine());

        System.out.println("Enter no. of generator bits:");
        gen_bits = Integer.parseInt(br.readLine()); // divisor jisse encode krre hai usmein total bits kitne hai
        ...
        total_bits = message_bits + gen_bits - 1; // dividend mein kitne bits hai
        app_message = new int[total_bits]; // codeword mein total kitne bits honge
    ...
    static int[] computecrc(int[] gen, int[] rem) { // division function
        int current = 0;
        int i;
        while(true) {
            for(i=0; i<gen.length; i++)
